...
### T02 — 配置 Prisma + Neon PostgreSQL 基本模型并能迁移

* **目标**：添加 Prisma schema、生成客户端，能成功运行 `prisma db push` 并连接到 Neon 云数据库。
* **文件/路径**：`prisma/schema.prisma`、`src/lib/prisma.ts`、`.env`
* **开始条件**：T01 完成，Neon 账号已注册。
* **结束条件**：Neon 数据库中存在表结构，`src/lib/prisma.ts` 导出 `prisma` 单例。
* **步骤**:

  1. 创建 `.env` 文件，配置 Neon DATABASE_URL。
  2. 写入 `schema.prisma`：定义 `User`, `Blog`, `Category`, `Tag`, `Session`, `Account`（保留最小字段）。
  3. 运行 `npx prisma generate` 生成客户端。
  4. 运行 `npx prisma db push` 推送表结构到 Neon。
  5. 在 `src/lib/prisma.ts` 导出单例 `PrismaClient`（避免多实例）。

* **验收测试**：

  * `npx prisma generate` 成功执行无错误。
  * `npx prisma db push` 成功连接并创建表结构。
  * 在 Neon 控制台能看到创建的表。
  * `src/lib/prisma.ts` 能正确导入和使用。

* **依赖**：T01
* **回滚**：在 Neon 控制台删除表，恢复 schema。
